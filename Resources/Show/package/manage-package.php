				...
				<div class="manage-employee card mx-auto">
					...
					<div class="card-body">
						<div class="action-list">
							<a class="btn btn-outline-secondary" href="http://localhost/MVC/manage/package/new" id="newEmployee">New Package</a>
						</div>		

						@if($employees)
							<form class="form-inline my-2 my-lg-0 p-3 clear-float" method="POST" action="http://localhost/MVC/manage/employee/search">
							   <input class="form-control mr-sm-2" placeholder="Search Record" type="text" name="pattern">		
							...
							@foreach($employees as $employee)
										<tr data="{{$employee->id}}">
											<td id="first_name">{{$employee->first_name}}</td>
											<td id="middle_name">{{$employee->middle_name}}</td>
											<td id="last_name">{{$employee->last_name}}</td>
											<td id="address">{{$employee->address}}</td>
											<td id="birthday">{{$employee->birthday}}</td>
											<td id="contact_number">{{$employee->contact_number}}</td>
											<td>
												<button class="btn btn-outline-info" id="showGraph">Show</button>
											</td>
						...
						@else
							...
							@else
								<div class="clear-float">
									<div class="alert alert-light">
										No Package's available. Click "New Package" to add.
									</div>					
								</div>
							@endif
						@endif
