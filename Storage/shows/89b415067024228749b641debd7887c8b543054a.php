					...
					<div class="card-body">
						...
						<?php if($transactions) : ?>				
							...
							<?php foreach($transactions as $transaction) : ?>
								    	<tr dataVehicle="<?php echo(esc($transaction->vehicle_id)); ?>" 
								    		dataTransaction="<?php echo(esc($transaction->id)); ?>">
											<td>
											<?php echo(esc($transaction->first_name)); ?>
											<?php echo(esc($transaction->middle_name[0])); ?>.
											<?php echo(esc($transaction->last_name)); ?>
											</td>
											<td id="status">
											<?php echo(esc($transaction->status)); ?>
											</td>
											<td>
											<?php echo(esc($transaction->date_time)); ?>
											</td>
						...
						<?php else: ?>
							...
							<?php else: ?>
								<div class="clear-float">
									<div class="alert alert-light">
										<div>
										No transaction available. Click "New Transaction" to generate transaction.
										</div>
									</div>						
								</div>
							<?php endif; ?> 
