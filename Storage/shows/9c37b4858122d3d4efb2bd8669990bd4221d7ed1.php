...
<script type="text/javascript">
	$(function() {
		//Graph for this year in every month
		$(document).on('click', '#showGraph', function() {
			var year = $(".table-responsive").attr('datayear');
				...
				$.ajax({
					...
					success : function(data) {
						$("tbody tr").each(function() {
							if ($(this).attr('data') == id) {
								$(this).after(data.graphTemplate);
								renderChartbyTimeline(id, start, end, 'day');
							}
						})
					}
				});
		...
		$(document).on('click', '#showWeekTimeline', function() {
			var id = $(this).closest('tr').attr('data');
			var start = $('.table-responsive').attr('datastart');
			var end = $('.table-responsive').attr('dataend');
			var title = $('.table-responsive').attr('dataweek');
				...
				$.ajax({
					...
					success : function(data) {
						$("tbody tr").each(function() {
							if ($(this).attr('data') == id) {
								$(this).after(data.graphTemplate);
								renderChartbyTimeline(id, start, end, 'week', title);
							}
						})
					}
				});
		...
		$(document).on('click', '#showMonthTimeline', function() {
			var id = $(this).closest('tr').attr('data');
			var start = $('.table-responsive').attr('datastart');
			var end = $('.table-responsive').attr('dataend');

			var date = new Date(start);
			var date = { 'month' : getMonthName(date.getMonth()), 'year' : getYear(date) };
				...
				$.ajax({
					...
					success : function(data) {
						$("tbody tr").each(function() {
							if ($(this).attr('data') == id) {
								$(this).after(data.graphTemplate);
								renderChartbyTimeline(id, start, end, 'month', date.month, date.year);
							}
						})
					}
				});
		...
		$(document).on('submit', '#generate', function() {
			window.location.href = "http://localhost/MVC/manage/report/statistics/from/" + $('#fromOption').val() + "/to/" + $('#toOption').val() + "/page/1";
			return false;
		});

		$(document).on('submit', '#generateWeek', function() {
			window.location.href = "http://localhost/MVC/manage/report/statistics/week/" + $('#weekOption').val() + "/page/1";
			return false;
		});

		$(document).on('submit', '#generateMonth', function() {
			window.location.href = "http://localhost/MVC/manage/report/statistics/month/" + $('#monthOption').val() + "/year/" + $('#yearOption').val() + "/page/1";
			return false;
		});

		$(document).on('submit', '#generateYear', function() {
			window.location.href = "http://localhost/MVC/manage/report/statistics/year/" + $('#yearOption').val() + "/page/1";
			return false;
		});
	});
</script>
					...
					<div class="card-body">
						...
						<?php if($statistics) : ?>
							...
							<?php endif; ?>
								...
								<?php if($byMonth) : ?>
								  <div class="table-responsive p-3" datayear="<?php echo(esc($byMonth['month'])); ?>"
								  datamonth="<?php echo(esc($byMonth['year'])); ?>">
								<?php endif; ?>

								<?php if($byYear) : ?>
								 <div class="table-responsive p-3" datayear="<?php echo(esc($byYear)); ?>">	
								<?php endif; ?>

								<?php if($byTimeline) : ?>
									<?php if($weekTitle) : ?>
									<div class="table-responsive p-3" dataweek="<?php echo(esc($weekTitle)); ?>"
									datastart="<?php echo(esc($byTimeline['start'])); ?>"
								...
								<?php foreach($statistics as $statistic) : ?>
								    	<tr data="<?php echo(esc($statistic->id)); ?>">
											<td>
											<?php echo(esc($statistic->last_name)); ?>
											</td>
											<td>
											<?php echo(esc($statistic->first_name)); ?>
											</td>
											<td>
											 <?php echo(esc($statistic->middle_name)); ?>
											</td>
											<td>
											<?php echo(esc($statistic->count)); ?>
											</td>
											<td class="text-center" style="width : 220px;">
												<button class="btn btn-outline-success" id="<?php echo(esc($graphType)); ?>">View Graph/Timeline</button>
											</td>
										</tr>	
								<?php endforeach;  ?>
									</table>
						...
						<?php else: ?>
							<div class="clear-float">
								<div class="alert alert-light">
									<div>
									No reports for <?php echo(esc($header)); ?>
									</div>
								</div>						
							</div>
						<?php endif; ?>
